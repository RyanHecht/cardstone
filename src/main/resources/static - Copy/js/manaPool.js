...
class manaPool{
	...
	static buildPool(drawLimit,seperator,element){
		let pool = new manaPool(drawLimit,seperator);
		pool.setFire(element.fire);
		pool.setWater(element.water);
		pool.setEarth(element.earth);
		pool.setAir(element.air);
		pool.setBalance(element.balance);
		return pool;
	}
