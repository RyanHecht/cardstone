...
public class Board implements Jsonifiable, Serializable {
  ...
  public Board(OrderedCardCollection deckOne, OrderedCardCollection deckTwo) {
    ...
    creatureOne = new OrderedCardCollection(Zone.CREATURE_BOARD,
        deckOne.getPlayer());
    ...
    creatureTwo = new OrderedCardCollection(Zone.CREATURE_BOARD,
        deckTwo.getPlayer());
    ...
    while (eventQueue.size() >= 1 || effectQueue.size() >= 1) {
      if (eventQueue.size() >= 1) {
        ...
        if (e.getType() == EventType.TURN_START) {
          ...
          addCardToOcc(activeDeck.getFirstCard(),
              getOcc(activePlayer, Zone.HAND), activeDeck);
        }
  ...
  public OrderedCardCollection getOcc(Player p, Zone z) {
    ...
    case PLAYER_ONE:
      ...
      default:
        throw new RuntimeException(
            "ERROR: Illegal Zone in getOcc (Board.java)");
      }
    case PLAYER_TWO:
      switch (z) {
      case CREATURE_BOARD:
    ...
    default:
      throw new RuntimeException(
          "ERROR: Illegal player enum in getOcc (Board.java)");
    }
  }
  ...
  public void setOcc(OrderedCardCollection cards) {
    ...
    case PLAYER_ONE:
      ...
      default:
        throw new RuntimeException(
            "ERROR: Illegal Zone in setOcc (Board.java)");
      }
      break;
    case PLAYER_TWO:
      switch (cards.getZone()) {
    ...
    default:
      throw new RuntimeException(
          "ERROR: Illegal player enum in setOcc (Board.java)");
    }
  }
  ...
  public Card getCardById(int id) {
    ...
    throw new IllegalArgumentException(
        "ERROR: No card with id" + id + " found.");
  }
  ...
  public JsonObject jsonifySelf() {
    ...
    OrderedCardCollection inactivePlayerCreatures = getOcc(getInactivePlayer(),
        Zone.CREATURE_BOARD);
  ...
  public void changeCreatureHealth(Creature target, int amount, Zone z) {
    StatChangeEvent event = new StatChangeEvent(EventType.HEALTH_CHANGE, target,
        amount);
   ...
   */
  public void addCardToOcc(Card c, OrderedCardCollection destination,
      OrderedCardCollection start) {
    ...
    if (destination.getZone() == Zone.HAND) {
      ...
      sendAnimation(animation);
    } else if (start.getZone() == Zone.HAND
        && destination.getZone() == Zone.CREATURE_BOARD
        && c.isA(Creature.class)) {
      JsonObject animation = new JsonObject();
      ...
      sendAnimation(animation);
        ...
        && destination.getZone() == Zone.AURA_BOARD && c.isA(AuraCard.class)) {
      JsonObject animation = new JsonObject();
      ...
      sendAnimation(animation);
    } else if (start.getZone() == Zone.AURA_BOARD
        && destination.getZone() == Zone.GRAVE && c.isA(AuraCard.class)) {
      JsonObject animation = new JsonObject();
      ...
      sendAnimation(animation);
        ...
        && destination.getZone() == Zone.GRAVE && c.isA(SpellCard.class)) {
      JsonObject animation = new JsonObject();
  ...
  public void changeCreatureAttack(Creature target, int amount, Zone z) {
    StatChangeEvent event = new StatChangeEvent(EventType.ATTACK_CHANGE, target,
        amount);
  ...
  public Zone getZoneOfCard(Card c) {
    ...
    throw new IllegalArgumentException(
        "Tried to get Zone of card that wasn't found in game");
  }
