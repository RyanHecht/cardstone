...
public class OrderedCardCollection implements CardCollection, Jsonifiable {
  ...
  public List<Effect> handleCardBoardEvent(Event event) {
    ...
    default:
      throw new RuntimeException(
          "ERROR: Invalid event type: " + event.getType());
    }
  ...
  private List<Effect> handleCardTargeted(CardTargetedEvent event) {
    List<Effect> results = new LinkedList<>();
    for (Card c : cards) {
      results.add(c.onCardTarget(event.getTargetter(), event.getTargeted(),
          getZone(), event.getTargetZone()));
    }
    return results;
  }
  ...
  private List<Effect> handlePlayerTargeted(PlayerTargetedEvent event) {
    ...
    for (Card c : cards) {
      results.add(
          c.onPlayerTarget(event.getTargetter(), event.getTarget(), getZone()));
    }
    return results;
  }
  ...
  private List<Effect> handleElementGain(GainElementEvent event) {
    List<Effect> results = new LinkedList<>();
    for (Card c : cards) {
      results.add(c.onElementGain(event.getPlayer(), event.getElementType(),
          event.getAmount(), getZone()));
    }
    return results;
  }
  ...
  private List<Effect> handleAttackChange(StatChangeEvent event) {
    ...
    for (Card c : cards) {
      results.add(
          c.onAttackChange(event.getTarget(), event.getAmount(), getZone()));
    }
    return results;
  }
  ...
  private List<Effect> handleHealthChange(StatChangeEvent event) {
    ...
    for (Card c : cards) {
      results.add(
          c.onHealthChange(event.getTarget(), event.getAmount(), getZone()));
    }
    return results;
  }
  ...
  private List<Effect> handleCardDamaged(CardDamagedEvent cDamaged) {
    List<Effect> results = new LinkedList<>();
    for (Card c : cards) {
      results.add(c.onCreatureDamage(cDamaged.getTarget(), cDamaged.getSrc(),
          cDamaged.getDmg(), getZone()));
    }
    return results;
  }
  ...
  private List<Effect> handlePlayerDamaged(PlayerDamagedEvent pDamaged) {
    List<Effect> results = new LinkedList<>();
    for (Card c : cards) {
      results.add(c.onPlayerDamage(pDamaged.getPlayer(), pDamaged.getSrc(),
          pDamaged.getDmg(), getZone()));
    }
    return results;
  }
  ...
  private List<Effect> handleCardHealed(CardHealedEvent cHealed) {
    List<Effect> results = new LinkedList<>();
    for (Card c : cards) {
      results.add(c.onCreatureHeal(cHealed.getTarget(), cHealed.getSrc(),
          cHealed.getHeal(), getZone()));
    }
    return results;
  }
  ...
  private List<Effect> handlePlayerHealed(PlayerHealedEvent pHealed) {
    List<Effect> results = new LinkedList<>();
    for (Card c : cards) {
      results.add(c.onPlayerHeal(pHealed.getTarget(), pHealed.getSrc(),
          pHealed.getHeal(), getZone()));
    }
    return results;
  }
  ...
  private List<Effect> handleCardZoneChange(CardZoneChangeEvent change) {
    List<Effect> results = new LinkedList<>();
    for (Card c : cards) {
      results.add(c.onZoneChange(change.getCard(), change.getEnd(),
          change.getStart(), getZone()));
    }
    return results;
  }
  ...
  private List<Effect> handleCardZoneCreated(CardZoneCreatedEvent created) {
    List<Effect> results = new LinkedList<>();
    for (Card c : cards) {
      results.add(c.onCardZoneCreated(created.getCard(), created.getLocation(),
          getZone()));
    }
    return results;
  }
  ...
  private List<Effect> handleCreatureAttacked(CreatureAttackEvent cAttack) {
    List<Effect> results = new LinkedList<>();
    for (Card c : cards) {
      results.add(c.onCreatureAttack(cAttack.getAttacker(), cAttack.getTarget(),
          getZone()));
    }
    return results;
  }
  ...
  private List<Effect> handlePlayerAttacked(PlayerAttackEvent pAttack) {
    List<Effect> results = new LinkedList<>();
    for (Card c : cards) {
      results.add(c.onPlayerAttack(pAttack.getAttacker(), pAttack.getTarget(),
          getZone()));
    }
    return results;
  }
  ...
  private List<Effect> handleCardChosen(CardChosenEvent event) {
    ...
    for (Card c : cards) {
      results.add(
          c.onCardChosen(event.getChooser(), event.getChosen(), getZone()));
    }
    return results;
  }
