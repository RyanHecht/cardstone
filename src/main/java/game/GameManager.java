...
public class GameManager {
   ...
   */
	...
	static void conditionalInsert(Game g) {
		...
		try (ResultSet rs = Db.query("select id from in_progress where id = ?;", gId)) {
			String serialG = g.serialize();
			if (!rs.next()) {
				System.out.println("I'm inserting");
				Db.update("insert into in_progress values(?, ?, ?, ?);", g.getId(), p1, p2, serialG);
			} else {
				System.out.println("I'm updating");
				Db.update("update in_progress set board = ? where player1 = ?;", serialG, p1);
			}
			System.out.println(String.format("Stashed game %d with players %d and %d in db", gId, p1, p2));
		} catch (IOException | SQLException | NullPointerException e) {
			System.out.println(String.format("Couldn't insert game %d with players %d and %d into db", gId, p1, p2));
			e.printStackTrace();
		}
	}
}
