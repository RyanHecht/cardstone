...
public class Game implements Jsonifiable {
  ...
  public Game(List<String> firstPlayerCards, List<String> secondPlayerCards,
    ...
    OrderedCardCollection deckOne = new OrderedCardCollection(Zone.DECK,
        playerOne);
    OrderedCardCollection deckTwo = new OrderedCardCollection(Zone.DECK,
        playerTwo);
    ...
    try {
      ...
      for (String formattedName : firstPlayerCards) {
        Object p = Class.forName(PACKAGE_PATH + formattedName)
            .getConstructor(Player.class).newInstance(playerOne);
      ...
      for (String formattedName : secondPlayerCards) {
        Object p = Class.forName(PACKAGE_PATH + formattedName)
            .getConstructor(Player.class).newInstance(playerTwo);
        if (p instanceof Creature) {
    ...
    if (!(isTurn(playerId))) {
      ...
      if (state != GameState.IDLE) {
        ...
        sendPlayerActionBad(playerId,
            "Please make a choice from the presented cards!");
        return;
      }
      ...
      try {
        ...
        CommsWebSocket.sendTextMessage(board.getActivePlayer().getId(),
            "It's your turn!");
    ...
    if (isTurn(playerId)) {
      ...
      if ((targetter instanceof Creature) && (targetee instanceof Creature)) {
        ...
        if (attacker.getOwner().getId() != playerId) {
          sendPlayerActionBad(playerId,
              "Can't attack using an opponents creature!");
          return;
        }
        ...
        if (!(board.getActivePlayer().validateCost(targetter.getCost()))) {
          sendPlayerActionBad(playerId,
              "Cannot play card, insufficient resources.");
          return;
        }
        ...
        CardTargetedEvent event = new CardTargetedEvent(
            (TargetsOtherCard) targetter, targetee);
        ...
        sendPlayerActionBad(playerId,
            "The card you attempted to use to target something isn't allowed to do that.");
      }
    } else {
      // player acting out of turn.
      sendPlayerActionBad(playerId, "Acting out of turn.");
      ...
      if (target) {
        ...
        if (card instanceof TargetsPlayer) {
          ...
          if (!(((TargetsPlayer) card)
              .playerValidTarget(board.getActivePlayer()))) {
            sendPlayerActionBad(playerId,
                "You can't target yourself with that card.");
            return;
          }

          // construct event.
          PlayerTargetedEvent event = new PlayerTargetedEvent(
              (TargetsPlayer) card, board.getActivePlayer());
        ...
        if (card instanceof Creature) {
          ...
          if (!(attacker.canAttack())) {
            sendPlayerActionBad(playerId,
                "That creature can no longer attack.");
            return;
          }
          ...
          PlayerAttackEvent event = new PlayerAttackEvent(
              board.getInactivePlayer(), attacker);
          ...
          if (!(((TargetsPlayer) card)
              .playerValidTarget(board.getInactivePlayer()))) {
            sendPlayerActionBad(playerId,
                "You can't target your opponent with that card.");
            return;
          }

          // construct event.
          PlayerTargetedEvent event = new PlayerTargetedEvent(
              (TargetsPlayer) card, board.getInactivePlayer());
          ...
          sendPlayerActionBad(playerId,
              "You can't target a player with that card.");
        }
      }
    } else {
      sendPlayerActionBad(playerId, "Acting out of turn.");
  ...
  public void handleChosen(JsonObject userInput, int playerId) {
    if (isTurn(playerId)) {
      ...
      CardPlayedEvent cEvent = new CardPlayedEvent(chooserCard,
          board.getOcc(chooserCard.getOwner(), Zone.GRAVE),
  ...
  public void handleCardPlayed(JsonObject userInput, int playerId) {
    ...
    if (isTurn(playerId)) {
      ...
      CardPlayedEvent event = new CardPlayedEvent(card,
  ...
  public JsonObject playerOneJsonify(JsonObject toMod) {
    toMod.get("board").getAsJsonObject().get("hand2").getAsJsonObject()
        .remove("cards");
    return toMod;
  }

  public JsonObject playerTwoJsonify(JsonObject toMod) {
    toMod.get("board").getAsJsonObject().get("hand1").getAsJsonObject()
        .remove("cards");
    return toMod;
  }
=======
	...
	public Game(List<String> firstPlayerCards, List<String> secondPlayerCards, int playerOneId, int playerTwoId) {
		...
		// one is a linked list one is an arraylist
		// for hybrid performance gains - Will Riley
		List<String> formattedListOne = new LinkedList<>();
		List<String> formattedListTwo = new ArrayList<>();

		// format the card names from the lists.
		for (String name : firstPlayerCards) {
			formattedListOne.add(name.replaceAll("[^A-Za-z0-9]", "").replaceAll(" ", ""));
		}

		for (String name : secondPlayerCards) {
			formattedListTwo.add(name.replaceAll("[^A-Za-z0-9]", "").replaceAll(" ", ""));
		}
		...
		try {
			...
			// invoking constructors as we go and adding to new list before adding all
			// to the OrderedCardCollections?
			for (String formattedName : formattedListOne) {
			...
			// repeat process with player two's deck.
			for (String formattedName : formattedListTwo) {
...
>>>>>>> 94a36ba5369df261e32d86bb1ed0a0007f63854e
}
