...
public class Game implements Jsonifiable, Serializable {
  ...
  public Game(List<String> firstPlayerCards, List<String> secondPlayerCards,
      int playerOneId, int playerTwoId, boolean isTutorial) {
    ...
    for (String name : secondPlayerCards) {
      formattedSecond
          .add(name.replaceAll("[^A-Za-z0-9]", "").replaceAll(" ", ""));
    }
    ...
    if (isTurn(playerId)) {
      ...
      if ((targetter instanceof Creature) && (targetee instanceof Creature)) {
        ...
        CardTargetedEvent event = new CardTargetedEvent(
            (TargetsOtherCard) targetter, targetee, targetIn);
